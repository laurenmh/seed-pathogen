## This script uses glmms for germination and pathogen abundance
## to quantify the variance within vs. among moms

library(corrplot)
library(lme4)
library(car)

####################################### Start with Festuca
Fsite<-read.csv('FesRoe Site data.csv')
Fdat<-read.csv('FesRoe germination and pathogens FINAL 2019_7_17.csv')

####### First do a PCA of Festuca climate variables
# can't have more variables than observations, so eliminate the most highly correlated from the pca
corrplot(cor(Fsite[,c('Latitude','LastyrElevation','SpringPPT','SpringTminC','SpringTmeanC','SpringTmaxC',
                      'SpringTdewmeanC','SpringVPDmin','SpringVPDmax','SoilPercentNitrogen')]),addCoef.col='black')
# pick 6 least-correlated variables and re-scale to 0 mean and 1 sd
Fsite_scaled<-apply(Fsite[,c('SpringPPT','SpringTminC','SpringTmeanC','SpringTdewmeanC','SpringVPDmin','SoilPercentNitrogen')],2,scale,scale=TRUE,center=TRUE)
Fpca<-princomp(Fsite_scaled[,c('SpringPPT','SpringTminC','SpringTmeanC','SpringTdewmeanC','SpringVPDmin','SoilPercentNitrogen')])
biplot(Fpca)
summary(Fpca)
# get first 3 principle components which together explain > 90% of variation
Fsite$PC1<-Fpca$scores[,1]
Fsite$PC2<-Fpca$scores[,2]
Fsite$PC3<-Fpca$scores[,3]
# what do these represent?
loadings(Fpca)
# PC1 is mostly an axis of wet and cold vs. dry and hot
# PC2 is mostly an axis of high vpd and soil N and low Tmean vs. low vpd and soil N and high Tmean
# PC3 is mostly an axis of high soil N and low vpd and Tmin vs. low soil N and high vpd and Tmin


###### Merge climate PCs into performance data set
# First make the population names consistent
unique(Fdat$SiteName)%in%unique(Fsite$?..SiteName) # not all the same
unique(Fdat$SiteName)
# Fill in missing site names
Fdat[which(Fdat$SiteName==''),]
Fdat$SiteName[Fdat$SiteName==''&!is.na(Fdat$IND)]<-"Upper Table Rock"
# Fix differences between data sets
unique(Fsite$?..SiteName)
Fdat$SiteName<-as.character(Fdat$SiteName)
Fdat$SiteName[Fdat$SiteName=='Hazel Dell Upper']<-"Hazel Dell-upper"
Fdat$SiteName[Fdat$SiteName=='Whidby Island']<-"Whidbey Island"
unique(Fdat$SiteName)%in%unique(Fsite$?..SiteName) # not all the same
# now merge datasets
Fdat<-merge(Fdat,Fsite,by.x=c('SiteName'),by.y=c('?..SiteName'))

###### Fit germination models
# germination as a function of climate PCs and plant density, with mom nested within pop as random effects
Fgerm<-glmer(Germ.~PC1+PC2+PC3+PlantDensity+(1|SiteName/SiteInd),family='binomial',data=Fdat,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(Fgerm)
# there is no variance explained by site after accounting for climate effects
# this is generating the 'singular fit' warning
Fgerm2<-glmer(Germ.~PC1+PC2+PC3+PlantDensity+(1|SiteInd),family='binomial',data=Fdat,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
anova(Fgerm,Fgerm2)
# the likelihood ratio test confirms that there is no support for including a random effect of site
Fgerm3<-glm(Germ.~PC1+PC2+PC3+PlantDensity,family='binomial',data=Fdat)
anova(Fgerm2,Fgerm3)
# however, the random effect of mom is highly significant, strong support for variation among moms in germination success
summary(Fgerm2)
as.data.frame(VarCorr(Fgerm2)) # 2.277 is the variance in germination due to moms
# there is a significant effect of the first two climate PCs, but not the third or plant density
# germination decreases with increasing PC1 (wetter and colder Tmin and Tmean)
# germination increases with increasing PC2 (higher soil N and VPDmin and colder Tmean)

###### Fit pathogen richness models
hist(Fdat$Nr..kinds) # model as poisson distributed
Fpath<-glmer(Nr..kinds~PC1+PC2+PC3+PlantDensity+(1|SiteName/SiteInd),data=Fdat,family='poisson',control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(Fpath)
# test significance of random effects with likelihood ratio test
Fpath2<-glmer(Nr..kinds~PC1+PC2+PC3+PlantDensity+(1|SiteInd),data=Fdat,family='poisson',control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
anova(Fpath,Fpath2) # Strong support for variation among sites (and convergence problems when site is removed from the model)
Fpath3<-glmer(Nr..kinds~PC1+PC2+PC3+PlantDensity+(1|SiteName),data=Fdat,family='poisson',control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
anova(Fpath,Fpath3) # Strong support for variation among moms within site (and convergence problems when mom is removed from the model)
# strong support for variation in pathogen richness among sites and among moms within sites
as.data.frame(VarCorr(Fpath)) # .00797 is the variance among moms and .01774 is the variance among sites in pathogen richness
summary(Fpath)
# there is no significant effect of climate or plant density on pathogen richness

# save Festuca workspace
save.image("~/HOPS project/seed-pathogen/Festuca glmms.RData")

############################################## Now do Danthonia
Dsite<-read.csv('Data/DANCAL site data FINAL 2019_07_19.csv')
Ddat<-read.csv('Data/Dan Cal pathogen and germination FINAL 2019_07_19.csv')

####### First do a PCA of Danthonia climate variables
# can't have more variables than observations, so eliminate the most highly correlated from the pca
corrplot(cor(Dsite[,c('Latitude','LastyrElevation','SpringPPT','SpringTminC','SpringTmeanC','SpringTmaxC',
                      'SpringTdewmeanC','SpringVPDmin','SpringVPDmax','X.Nitrogen')],
             use='pairwise.complete.obs'),addCoef.col='black')
# latitude is highly correlated with soil N and available for all sites
# so drop soil N and re-scale to 0 mean and 1 sd
Dsite_scaled<-apply(Dsite[,c('Latitude','LastyrElevation','SpringPPT','SpringTminC','SpringTmeanC','SpringTmaxC',
                             'SpringTdewmeanC','SpringVPDmin','SpringVPDmax')],2,scale,scale=TRUE,center=TRUE)
Dpca<-princomp(Dsite_scaled[,c('Latitude','LastyrElevation','SpringPPT','SpringTminC','SpringTmeanC','SpringTmaxC',
                               'SpringTdewmeanC','SpringVPDmin','SpringVPDmax')])
biplot(Dpca)
summary(Dpca)
# get first 3 principle components which together explain > 94% of variation
Dsite$PC1<-Dpca$scores[,1]
Dsite$PC2<-Dpca$scores[,2]
Dsite$PC3<-Dpca$scores[,3]
# what do these represent?
loadings(Dpca)
# PC1 is mostly an axis of cool/wet/high latitude vs. warm/dry/low latitude
# PC2 is mostly an axis of cool/wet/high elevation vs. warm/dry/low elevation
# PC3 is mostly an axis of high Tmin/high VPDmin vs. low Tmin/low VPDmin


###### Merge climate PCs into performance data set
# First make the population names consistent
unique(Ddat$SiteName)%in%unique(Dsite$SiteName) # all the same
# now merge datasets
Ddat<-merge(Ddat,Dsite,by.x=c('SiteName'),by.y=c('SiteName'))

###### Fit germination models
# germination as a function of climate PCs and plant density, with mom nested within pop as random effects
Dgerm<-glmer(germ.~PC1+PC2+PC3+PlantDensity+SeedType+(1|SiteName/SiteInd),family='binomial',data=Ddat,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(Dgerm)
# there is no variance explained by site after accounting for climate effects
# this is generating the 'singular fit' warning
Dgerm2<-glmer(germ.~PC1+PC2+PC3+PlantDensity+SeedType+(1|SiteInd),family='binomial',data=Ddat,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
anova(Dgerm,Dgerm2)
# the likelihood ratio test confirms that there is no support for including a random effect of site
Dgerm3<-glm(germ.~PC1+PC2+PC3+PlantDensity+SeedType,family='binomial',data=Ddat)
anova(Dgerm2,Dgerm3)
# however, the random effect of mom is highly significant, strong support for variation among moms in germination success
summary(Dgerm2)
as.data.frame(VarCorr(Dgerm2)) # 0.177 is the variance in germination due to moms
# there is a significant effect of the first and third climate PCs, and plant density
# but no effect of the second climate PC or seed type
# germination decreases with increasing PC1 (higher latitude, cooler Tmax, lower VPDmax)
# germination decreases with increasing PC3 (higher Tmin and higher VPDmin)
# germination decreases wtih increasing plant density

###### Fit pathogen richness models
hist(Ddat$NR..Kinds) # model as poisson distributed
Dpath<-glmer(NR..Kinds~PC1+PC2+PC3+PlantDensity+SeedType+(1|SiteName/SiteInd),data=Ddat,family='poisson',control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(Dpath)
# no variance due to site, this is what's causing the singular fit warning
# test significance of random effects with likelihood ratio test
Dpath2<-glmer(NR..Kinds~PC1+PC2+PC3+PlantDensity+SeedType+(1|SiteInd),data=Ddat,family='poisson',control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
# failure to converge, so re-start from the current parameter estimates
ss <- getME(Dpath2,c("theta","fixef"))
Dpath2 <- update(Dpath2,start=ss,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
# try again
ss <- getME(Dpath2,c("theta","fixef"))
Dpath2 <- update(Dpath2,start=ss,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
# it worked! 
anova(Dpath,Dpath2) # No support for variation among sites 
Dpath3<-glm(NR..Kinds~PC1+PC2+PC3+PlantDensity+SeedType,data=Ddat,family='poisson')
anova(Dpath2,Dpath3) # Strong support for variation among moms 
# strong support for variation in pathogen richness among moms but not sites
as.data.frame(VarCorr(Dpath2)) # .0568 is the variance among moms in pathogen richness
summary(Dpath2)
# there is a significant effect of the second and third climate PCs and seed type
# but no effect of the first climate PC or of plant density
# pathogen richness decreases with increasing PC2 (higher PPT and cooler Tmean)
# pathogen richness increases with increasing PC3 (higher Tmin and higher VPDmin)
# pathogen richness is lower for cleistogamous seeds than for chasmogamous seeds


Ddat2 <- Ddat %>%
  tbl_df() %>%
  group_by(SiteName, SiteInd, SeedType) %>%
  mutate(avgRich = mean(NR..Kinds, na.rm = T)) %>%
  mutate(perGerm = sum(germ.)/n()) %>%
  tbl_df()

Ddatc <- Ddat %>%
  select(SiteName, PlantDensity) %>%
  unique()
Ddat3 <- Ddat2 %>%
  select(SiteName, SiteInd, SeedType, PC1, PC2, PC3, PlantDensity, avgRich, perGerm) %>%
  unique()
  
Dgerm2<-glmer(germ.~PC1+PC2+PC3+PlantDensity + avgRich+SeedType+(1|SiteInd),family='binomial',data=Ddat2,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(Dgerm2)

Dgerm2x<-lmer(perGerm~PC1+PC2+PC3+PlantDensity + avgRich+SeedType+(1|SiteName),data=Ddat2)
summary(Dgerm2x)

Dgerm2x<-lmer(perGerm~PC1+PC2+PC3+PlantDensity + avgRich+SeedType+(1|SiteName),data=Ddat2)
summary(Dgerm2x)



model <- '
level: 1
perGerm ~ avgRich

level: 2
perGerm~  PC1 + PC2 + PlantDensity
PlantDensity ~ PC1 + PC2
avgRich ~ PC1 + PC2
PlantDensity ~~ avgRich
'


fit1 <- sem(model,std.ov=T,missing="ml", data=mydat2, cluster = "SiteName") #this uses robust chi square, which I've read is good and conservative
summary(fit1, fit.measures=TRUE,rsquare=T)


model <- '
level: 1
perGerm ~ avgRich

level: 2
perGerm~  PC1 + PC2 + PlantDensity
PlantDensity ~ PC1 + PC2
avgRich ~ PC1 + PC2
PlantDensity ~~ avgRich
'


fit1 <- sem(model,std.ov=T,missing="ml", data=mydat2, cluster = "SiteName") #this uses robust chi square, which I've read is good and conservative
summary(fit1, fit.measures=TRUE,rsquare=T)


# save Danthonia workspace
save.image("~/HOPS project/seed-pathogen/Danthonia glmms.RData")

